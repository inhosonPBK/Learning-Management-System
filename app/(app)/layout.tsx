...
export default async function AppLayout({ children }: LayoutProps<"/">) {
  ...
  if (viewer.profile.team_code) {
    const { data } = await createAdminClient()
      .from("teams")
      .select("name_ko, name_en")
      .eq("code", viewer.profile.team_code)
      .maybeSingle<{ name_ko: string | null; name_en: string }>();
    teamName = data ? (viewer.profile.locale === "ko" ? data.name_ko ?? data.name_en : data.name_en) : null;
  }
